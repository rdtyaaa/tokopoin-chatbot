...
class WithdrawService extends Controller
{
     ...
     */
    public function getPaginatedList(User $user) :LengthAwarePaginator{
     ...
     * @return array
     */
    public function createRequest(Request $request , User $user) : array {
    ...
    public function store (Request $request , User $user) :array {
        ...
        $withdraw =  Withdraw::where('id', $request->input('id'))
                            ->whereNull('deliveryman_id')
                            ->whereNull('seller_id')
                             ->where('status',  Withdraw::INITIATE)->where('user_id', $user->id)
                             ->first();

        if(!$withdraw)  return [ 'status'  => false, 'message' => translate("Invalid withdraw request")];

        if($withdraw->amount > $user->balance) return [ 'status'  => false, 'message' => translate("You do not have sufficient balance for withdraw.")];
