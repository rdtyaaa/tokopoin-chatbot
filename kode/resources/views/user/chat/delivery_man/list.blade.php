                                        ...
                                        <div class="chat-left-sidebar">
                                            ...
                                            <div class="session-list">
                                                ...
                                                @forelse ($deliverymen as $deliveryman)
                                                  ...
                                                  @php
                                                     $isSeen = true;
                                                     if($deliveryman->latestConversation && $deliveryman->latestConversation->sender_role ==  'deliveryman' && $deliveryman->latestConversation->is_seen == 0 ){
                                                        $isSeen = false;
                                                     }
                                                  ...
                                                  @endphp
                                                    ...
                                                    <div class="session-single  get-chat {{ !$isSeen ? "unread-message" :""}}  " id="{{ $deliveryman->id }}">
                                                        ...
                                                        <div class="content">
                                                            <div
                                                                ...
                                                                <div class="time">
                                                                    {{ $deliveryman->latestConversation->created_at->diffForHumans()}}
                                                                </div>
                                                            </div>
                                                            <div
                                                                class="d-flex justify-content-between align-items-center flex-wrap">
                                                                <p>{{ $deliveryman->latestConversation->message }}</p>
                                                                @if(!$isSeen)
                                                ...
                                                @empty
                                                    @include("frontend.partials.empty",['message' => translate('No Deliveryman found')])
                                                @endforelse
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-lg-9 position-relative chat-message-section">
                                        <div id="message" class="chat-message">
                                            <div class="empty-message">
                                                @include("frontend.partials.empty",['message' => translate('Please select a deliveryman')])
                                            </div>
...
@push('scriptpush')
    ...
    <script>
        ...
        $(document).ready(function() {
           if('{{request()->query("deliveryman_id")}}'){
             getMessage('{{request()->query("deliveryman_id")}}');
           }
        });
        ...
        $(document).on('click', '.get-chat', function(e) {
            ...
            getMessage(deliverymanId)

        });


        function getMessage(deliverymanId ,loader = true,page = null , scroll = true ,append = false){

            var url = "{{ route('user.deliveryman.chat.message', ':deliveryman_id') }}"
                .replace(':deliveryman_id', deliverymanId) ;
            if(page){
                url = url+'?page=' + page
            }
            $.ajax({
        ...
        $(document).on('submit', '#chatinput-form', function(e) {
            ...
            $.ajax({
                method: 'POST',
                url: "{{ route('user.deliveryman.chat.send_message') }}",
                beforeSend: function() {
                ...
                success: function(response) {
                    ...
                    if(response.status){
                        getMessage(response.deliveryman_id,false)
                    }else{
