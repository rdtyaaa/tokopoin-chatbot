            ...
            <div class="card" id="orderList">
                ...
                <div class="card-body pt-0">
                    <ul class="nav nav-tabs nav-tabs-custom nav-primary mb-3" role="tablist">
                        <li class="nav-item">
                            <a class="nav-link {{ request()->routeIs('admin.seller.order.index') ? 'active' : '' }} All py-3"
                                id="All" href="{{ route('admin.seller.order.index') }}">
                        ...
                        <li class="nav-item">
                            <a class='nav-link {{ request()->routeIs('admin.seller.order.pending') ? 'active' : '' }}  Placed py-3'
                                id="pending" href="{{ route('admin.seller.order.pending') }}">
                        ...
                        <li class="nav-item">
                            <a class="nav-link {{ request()->routeIs('admin.seller.order.placed') ? 'active' : '' }}  Placed py-3"
                                id="Placed" href="{{ route('admin.seller.order.placed') }}">
                        ...
                        <li class="nav-item">
                            <a class="nav-link {{ request()->routeIs('admin.seller.order.confirmed') ? 'active' : '' }} Confirmed py-3"
                                id="Confirmed" href="{{ route('admin.seller.order.confirmed') }}">
                        ...
                        <li class="nav-item">
                            <a class="nav-link Processing {{ request()->routeIs('admin.seller.order.processing') ? 'active' : '' }}   py-3"
                                id="Processing" href="{{ route('admin.seller.order.processing') }}">
                                <i class="ri-refresh-line me-1 align-bottom"></i>
                                {{ translate('Processing') }}
                            </a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link Processing {{ request()->routeIs('admin.seller.order.shipped') ? 'active' : '' }}   py-3"
                                id="shipped" href="{{ route('admin.seller.order.shipped') }}">
                        ...
                        <li class="nav-item">
                            <a class="nav-link py-3 {{ request()->routeIs('admin.seller.order.delivered') ? 'active' : '' }}   Delivered"
                                id="Delivered" href="{{ route('admin.seller.order.delivered') }}">
                        ...
                        <li class="nav-item">
                            <a class="nav-link {{ request()->routeIs('admin.seller.order.cancel') ? 'active' : '' }}   py-3 Returns"
                                id="Returns" href="{{ route('admin.seller.order.cancel') }}">
                        ...
                        <li class="nav-item">
                            <a class='nav-link {{ request()->routeIs('admin.seller.order.return') ? 'active' : '' }}   py-3 Returns'
                                id="Returns" href="{{ route('admin.seller.order.return') }}">
                                ...
                                @forelse($orders as $order)
                                    <tr>
                                        <td data-label="{{ translate('Order Number') }}">
                                            ...
                                            <a href="{{ route('admin.seller.order.details', $order->id) }}"
                                        ...
                                        <td data-label="{{ translate('Customer Info') }}" class="text-align-left">
                                           ...
                                           <span>{{ translate('Name') }}:
                                               {{ @$order->customer->name ?? @$firstName }}</span><br>
                                           <span>{{ translate('Phone') }}:
                                               {{ @$order->customer->phone ?? @$phone }}</span>
                                           <br>
                                           @if ($order->customer_id)
                                               <a href="{{ route('admin.customer.details', $order->customer_id) }}"
                                           ...
                                           <span class="badge bg-primary">{{ translate('Shipping Method') }}:
                                               {{ @$order->shipping->name ?? 'N\A' }}</span> <br>
                                       </td>
                                        <td data-label="{{ translate('Product Details') }}" class="tex-align-left">

                                            <button data-details ="{{ $order->orderDetails }}"
                                                class="ms-2  order-btn btn btn-info btn-sm custom-toggle active"
                                                data-bs-toggle="modal" id="order-btn"
                                                data-bs-target="#orderDetails">{{ translate('view All') }}
                                        ...
                                        <td data-label="{{ translate('Amount') }}">
                                            <span class="fw-bold">
                                                {{ short_amount($order->amount) }}</span><br>
