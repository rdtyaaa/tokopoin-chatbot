...
<head>
    ...
    <link rel="shortcut icon"
        href="{{ show_image('assets/images/backend/logoIcon/' . site_settings('site_favicon'), file_path()['favicon']['size']) }}"
        type="image/x-icon">
...
<body>
        ...
        <div class="preloader-wrapper">
            ...
            <div class="loader-content">
                <div class="loader-img scale-up-center">
                    <img src="{{ show_image('assets/images/backend/logoIcon/' . site_settings('site_logo'), file_path()['site_logo']['size']) }}"
                        alt="site-logo.jpg" class="img-fluid">
                </div>
            </div>
        </div>
    @endif
    ...
    <main>
        ...
        @includeWhen($news_latter->status == '1', 'frontend.section.news_later', [
    ...
    @if (site_settings('whats_app_plugin') == App\Enums\StatusEnum::true->status())
        <div class="wp-floating-btn">
            <a href="https://wa.me/{{ site_settings('whats_app_number') }}?text={{ site_settings('whats_app_number_int_message') }}"
                target="_blank">
                <span class="wp-icon">
        ...
        $(document).on('click', '.attribute-select', function(e) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    const price = response_data.discount_price === 0 ? response_data.price :
                        response_data.discount_price;
                    const priceHtml = `<span>${currencySymbol}${price}</span>`;
                    const discountHtml = response_data.discount_price !== 0 ?
                        ` <del>${currencySymbol}${response_data.price}</del>` : '';
    ...
    <script>
        ...
        socket.on('connect', function() {
            ...
            const userId = '{{ auth()->user()->id }}'; // Menambahkan user ID untuk subscribe ke channel
            socket.emit('join', `chat-channel.${userId}`);
        });

        // Menerima pesan dari WebSocket
        socket.on('new-message', function(message) {
            console.log('New message received:', message);
            // Update UI (misalnya, tampilkan pesan baru di chat)
